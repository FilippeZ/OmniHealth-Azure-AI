...
usecases = [
    ...
    {
        "patient_id": "PX-8814",
        "patient_name": "Sofia Karagiani",
        "notes": "PATIENT: Karagiani Sofia | AGE: 67 | ADMISSION: 2026-07-15. Clinical summary: Right knee pain during weight bearing, joint stiffness, crepitus. X-ray right knee confirms severe medial joint space narrowing and subchondral sclerosis. Diagnosis: Osteoarthritis of Right Knee (Grade III - M17.11)."
    }
]

output_dir = os.path.join(os.getcwd(), "usecase_outputs")
os.makedirs(output_dir, exist_ok=True)

results = []

print("==========================================================================")
print("🚀 OMNIHEALTH AI: EXECUTING 5 CLINICAL USE CASES (END-TO-END BENCHMARK)")
print("==========================================================================\n")
...
for uc in usecases:
    ...
    # Save FLUX image if present
    img_filepath = None
    if b64_img:
        try:
            img_bytes = base64.b64decode(b64_img)
            img_filepath = os.path.join(output_dir, f"{pid}_FLUX2_Illustration.png")
            with open(img_filepath, "wb") as f:
                f.write(img_bytes)
            print(f"📷 FLUX.2-pro image saved: {img_filepath}")
        except Exception as e:
            print(f"⚠️ Failed to decode b64 image: {e}")
    ...
    appr_payload = json.dumps({
        ...
        "decision": "APPROVED",
        "physician_notes": f"APPROVE: Verified digitized record for {name} ({pid}). Visual illustration shared with patient."
    }).encode("utf-8")
...
print("==========================================================================")
print("📊 BENCHMARK COMPLETE: SUMMARY OF 5 CLINICAL USE CASES")
print("==========================================================================\n")
...
for res in results:
    ...
    print(f"  Image File: {res['image_file'] or 'Generated (Base64 Rendered)'}")
